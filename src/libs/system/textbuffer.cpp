...
void TextBuffer::dump() {
    fprintf(stderr, "TextBuffer @ 0x%p, cursor: %zu\n", this, _cur);
    for (char c : _data) {
        fputc(c, stderr);
    }
    fputc('\n', stderr);
...
void TextBuffer::write(StringRef s) {
...
StringRef TextBuffer::readlineReverse() {
    size_t end = tell();
...
bool TextBuffer::search(StringRef sub) {
    size_t index = StringRef(&_data[_cur], _data.size() - _cur).find(sub);
...
bool TextBuffer::rsearch(StringRef sub) {
    size_t index = StringRef(&_data[0], _cur).rfind(sub);
    if (index == StringRef::npos) {
        _cur = 0;
        return false;
    }
