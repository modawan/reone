...
void Font::render(StringRef text, const glm::vec3 &position, const glm::vec3 &color, TextGravity gravity) {
    if (text.empty()) {
        return;
    }
...
glm::vec2 Font::getTextOffset(StringRef text, TextGravity gravity) const {
...
float Font::measure(StringRef text) const {
    float w = 0.0f;
