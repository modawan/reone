...
void AttackObjectAction::execute(std::shared_ptr<Action> self, Object &actor, float dt) {
    ...
    if (_state == Move) {
        if (!attacker.navigateTo(_target->position(), true, attacker.getAttackRange(), dt)) {
            return;
        }
        _state = Wait;
    }
    ...
    if (_state == Wait && round.canExecute(*this)) {
        _state = Attack;
    ...
    if (_state == Attack && _time >= kAttackDamageDelay) {
        _state = Hit;
        // TODO: remove requirement on shared_ptr for the attacker.
        _attacks.applyEffects(_game.getObjectById<Creature>(attacker.id()), *_target, _game);
    }
