...
PresentationGUI::PresentationGUI(resource::GameID gameId,
                                 const graphics::GraphicsOptions &options,
                                 PresentationServices services) :
    _gameId(gameId),
    _graphicsOptions(options),
    _presentation(services) {
...
void PresentationGUI::init() {
    if (_resRef.empty()) {
        throw std::logic_error("GUI resRef must not be empty");
    }
    _gui = _presentation.guis.get(_resRef, std::bind(&PresentationGUI::preload, this, std::placeholders::_1));
    if (!_gui) {
...
void PresentationGUI::preload(IGUI &gui) {
...
bool PresentationGUI::handle(const input::Event &event) {
    if (!_gui) {
        return false;
    }
...
void PresentationGUI::update(float dt) {
    if (_gui) {
        _gui->update(dt);
    }
}

void PresentationGUI::render() {
    if (_gui) {
        _gui->render();
    }
}

void PresentationGUI::loadBackground(BackgroundType type) {
    std::string resRef;
    ...
    if (_gui) {
        _gui->setBackground(_presentation.textures.get(resRef, TextureUsage::MainTex));
    }
}

void PresentationGUI::useBakedItemSlotArt(ListBox &listBox) {
    if (isTSL()) {
        return;
    }
...
void PresentationGUI::centerRootInCanvas(int canvasWidth, int canvasHeight) {
    auto &root = _gui->rootControl();
...
std::shared_ptr<Texture> PresentationGUI::itemFrameTexture(int stackSize) const {
    ...
    auto resRef = isTSL() ? kTSLFrames[tier] : kK1Frames[tier];
    return _presentation.textures.get(resRef, TextureUsage::GUI);
}

void PresentationGUI::tintK2InGameFooter() {
    if (!isTSL()) {
        return;
    }
...
void PresentationGUI::tintK2InGameHeader() {
    if (!isTSL()) {
        return;
    }
...
void PresentationGUI::enableK2ButtonBodyFill(Control &control) {
    if (!isTSL()) {
        return;
    }

    auto edge = _presentation.textures.get("uibit_brdr_16wet", TextureUsage::GUI);
    auto corner = _presentation.textures.get("uibit_brdr_16wct", TextureUsage::GUI);
...
void PresentationGUI::enableK2ButtonBodyFill(const std::shared_ptr<Control> &control) {
    if (!control) {
        return;
    }
...
void PresentationGUI::fillK2SectionStrip(const std::shared_ptr<Control> &topBar, const std::shared_ptr<Control> &bottomBar) {
    static constexpr int kLineThickness = 2;

    if (!isTSL() || !topBar || !bottomBar) {
        return;
    }
...
void PresentationGUI::useK2ShellTitle(const std::shared_ptr<Control> &control) {
    if (!isTSL() || !control) {
        return;
    }
...
void PresentationGUI::updateK2FilterButton(const std::shared_ptr<Control> &button, bool selected) {
    if (!button) {
        return;
    }
...
std::string PresentationGUI::guiResRef(const std::string &base) const {
    return isTSL() ? base + "_p" : base;
}

void PresentationGUI::onClick(const std::string &control) {
    auto clip = _presentation.sounds.getOnClick();
    _audioSource = _presentation.mixer.play(std::move(clip), AudioType::Sound);
}

void PresentationGUI::onSelectionChanged(const std::string &control, bool selected) {
    if (selected) {
        auto clip = _presentation.sounds.getOnEnter();
        _audioSource = _presentation.mixer.play(std::move(clip), AudioType::Sound);
    }
}
