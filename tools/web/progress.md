...
- **Concurrency trap (fixed):** `cancel-in-progress: false` left the first `queued` run holding the concurrency group so later runs stayed `pending` for 20+ minutes. Workflow **concurrency block removed** (2026-05-28) so queued jobs are not serialized behind a stuck group.
