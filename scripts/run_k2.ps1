...
if (-not $GameDir) {
    Write-Error "Pass -GameDir or set KOTOR2_DIR to a legal KOTOR 2 install directory."
    exit 2
}

$GameDir = (Resolve-Path $GameDir).Path
$marker = Join-Path $GameDir "swkotor2.exe"
if (-not (Test-Path $marker)) {
    Write-Error "KOTOR 2 marker executable was not found: $marker"
    exit 2
}
